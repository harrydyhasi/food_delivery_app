...
const updateCustAddress = AsyncHandler(async (req, res) => {
  const { orderId } = req.params; 
  ...
  if (!newAddress) {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .json(ApiResponse("New address is required", null, StatusCodes.BAD_REQUEST));
  }

  try {
    const updatedOrder = await OrderService.updateCustAddress(orderId, newAddress);
    res
      .status(StatusCodes.OK)
      .json(
        ApiResponse("Customer address updated successfully", updatedOrder, StatusCodes.OK)
      );
  } catch (error) {
    res
      .status(StatusCodes.NOT_FOUND)
